...
export class ToDoCommand implements ISlashCommand {
    ...
    public async executor(context: SlashCommandContext, read: IRead, modify: IModify, http: IHttp, persis: IPersistence): Promise<void> {
        ...
        if (!args?.length) {
            await this.sendNotification(context, modify, 'Usage: /todo some task, or /todo LISTALL, or /todo DELETE some task, or /todo DELETEALL');
        } else if (args[0] === 'DELETEALL') {
            const msg = sender + ' deleted all tasks: \n\n' + tasks.join('\n');
            await this.sendMessage(context, modify, msg);
            ...
            if (tasks.includes(toDelete)) {
                ...
                await this.sendNotification(context, modify, 'There is no task "' + toDelete + '", please make sure the exact task name is used');
            }
        } else if (args[0] === 'LISTALL') {
            const msg = 'Current tasks: \n\n' + tasks.join('\n');
            await this.sendNotification(context, modify, msg);
        } else {
    ...
    private async sendMessage(context: SlashCommandContext, modify: IModify, messageText: string): Promise<void> {
        await modify
            .getCreator()
            .finish(await modify
                .getCreator()
    ...
    private async sendNotification(context: SlashCommandContext, modify: IModify, messageText: string): Promise<void> {
        const room = context.getRoom();
        await modify
            .getNotifier()
